...
func diagonalCheck(board [6][7]string, column int, turn string) bool {
	...
	for _, position := range positions {
		...
		if startRow-position-3 >= 0 && startRow-position <= 5 {
			for i := 0; i < 4; i++ {
				// fmt.Printf(" i%v, column: %v, row: %v, boardSquare: %v\n", i, column+position+i, startRow-position-i, board[startRow][column+position+i])
		...
		if startRow+position >= 0 && startRow+position+3 <= 5 {
			for i := 0; i < 4; i++ {
				//				fmt.Printf(" i%v, column: %v, row: %v, boardSquare: %v\n", i, column+position+i, startRow+position+i, board[startRow+position+i][column+position+i])
				if board[startRow+position+i][column+position+i] != turn {
					break
				}
				if i == 3 {
